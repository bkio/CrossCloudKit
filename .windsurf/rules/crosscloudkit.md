...
| `IVectorService` | Vector upsert, similarity search, metadata filtering | Qdrant, Basic (file-based) |
...
if (result.IsSuccessful && result.Data.FinishReason == LLMFinishReason.ToolCall)
{
    ...
    messages.Add(new LLMMessage { Role = LLMRole.Assistant, Content = result.Data.Content, ToolCalls = result.Data.ToolCalls });
